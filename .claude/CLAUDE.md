...
**Use Spotify's exact code, and check it.** Resolution only ever goes more-specific → less-specific, so a file keyed `pl-PL` is unreachable when Spotify reports `pl` — those users silently get English. That is what happened to Polish: `pl-PL` worked while the language came from `navigator` (browsers report a region) and broke the moment it came from Spotify. Spotify's own locale list is the authority, and it is on disk:

```bash
ls /Applications/Spotify.app/Contents/Resources/Apps/xpui/i18n/   # 75 locales
```

Audit every file against it after touching this area — an exact `<code>.json` there means the key is reachable. Spotify ships `fr-CA`, `pt-BR` and `zh-CN` but plain `pl`, `nb`, `ca` and `is`.
